...
function Calendar({ selectedDate, onSelectDate }) {
  const [viewDate, setViewDate] = useState(selectedDate || new Date(2026, 2, 13)) // March 2026
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <button type="button" onClick={prevMonth} className="p-1 rounded transition-colors duration-200 hover:bg-gray-100 text-gray-600" aria-label="Previous month">
          ‹
        </button>
        <span className="font-semibold text-gray-800">{MONTHS[viewDate.getMonth()]} {viewDate.getFullYear()}</span>
        <button type="button" onClick={nextMonth} className="p-1 rounded transition-colors duration-200 hover:bg-gray-100 text-gray-600" aria-label="Next month">
          ›
        </button>
      </div>
        ...
        {calendarDays.map(({ date, currentMonth }) => (
          <button
            ...
            className={`py-2 text-sm rounded-lg transition ${
              !currentMonth ? 'text-gray-300' : isSameDay(date, selectedDate)
...
export default function Booking() {
  const [selectedDate, setSelectedDate] = useState(new Date(2026, 2, 13)) // March 13, 2026
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        {/* Step 2: Select Trainer — descriptions visible before selection */}
        <div className="space-y-4">
            ...
            {TRAINERS.map((trainer) => (
              <button
                ...
                className={`w-full text-left rounded-xl border-2 p-4 transition-all duration-200 ${
                  selectedTrainer?.id === trainer.id ? 'border-gray-400 bg-gray-100' : 'border-gray-200 hover:border-gray-300 bg-white'
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="inline-block mt-0.5 px-2 py-0.5 rounded-md bg-gray-200 text-gray-700 text-xs font-medium">
      ...
      {/* Booking Summary — shown when date, trainer, and time are all selected */}
      {selectedDate && selectedTrainer && selectedTime && (
        <section className="mt-12 pt-10 border-t border-gray-200/80">
          <div className="max-w-xl mx-auto">
            ...
            <button
              type="button"
              onClick={() => { /* confirm action */ alert(`Booking confirmed: ${selectedDate.toLocaleDateString()} with ${selectedTrainer.name} at ${selectedTime}`) }}
