    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                activeCategory === cat ? 'bg-primary-600 text-white' : 'bg-card border border-gray-200 text-gray-700 hover:bg-primary-100'
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-12">
        {filteredVideos.map((video) => (
          <article key={video.id} className="bg-card rounded-xl border border-gray-200/80 overflow-hidden shadow-sm flex flex-col transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg">
            <div className="relative h-44 sm:h-48 bg-gray-200 shrink-0 overflow-hidden">
              <img src={video.image} alt="" className="w-full h-full object-cover object-center" loading="lazy" />
              <span className="absolute top-3 right-3 px-2.5 py-0.5 rounded-md bg-primary-600 text-white text-xs font-medium">{video.category}</span>
            </div>
            <div className="p-4 flex flex-col flex-1">
              ...
              <button type="button" onClick={() => setSelectedVideo(video)} className="mt-auto w-full py-2.5 rounded-xl bg-primary-600 text-white font-medium text-sm transition-all duration-200 hover:bg-primary-700 hover:scale-[1.02] hover:shadow-md">
                Watch Now
              </button>
            </div>
          </article>
      ...
      {filteredVideos.length === 0 && <p className="text-gray-500 text-center py-12">No videos match your search or filter.</p>}

      {selectedVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setSelectedVideo(null)} role="dialog" aria-modal="true" aria-labelledby="video-modal-title">
          <div className="bg-card rounded-2xl border border-gray-200 shadow-xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="relative h-48 bg-gray-200">
              <img src={selectedVideo.image} alt="" className="w-full h-full object-cover" />
              <span className="absolute top-3 right-3 px-2.5 py-0.5 rounded-md bg-primary-600 text-white text-xs font-medium">{selectedVideo.category}</span>
            </div>
            <div className="p-5">
              ...
              <button type="button" onClick={() => setSelectedVideo(null)} className="w-full py-2.5 rounded-xl bg-primary-600 text-white font-medium hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
                Close
              </button>
            </div>
          </div>
